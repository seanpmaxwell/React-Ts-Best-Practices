...
- `useState` is fine for components with one or two state variables. As state grows, switch to a custom hook (such as [`useSetState`](https://github.com/seanpmaxwell/js-dumping-groud/blob/main/hooks/useSetState.ts)) that manages a single state object.
